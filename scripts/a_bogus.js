// Douyin a_bogus signature algorithm.
// Adapted from https://github.com/zouyi741599086/douyinlive/blob/main/node/js/a_bogus.js
// Used by app/Parsers/DouyinParser.php through a small Node CLI wrapper.
...
if (require.main === module) {
    ...
    const ua = readArg('--ua');
    if (!query || !ua) {
        console.error('Usage: node a_bogus.js --query <url-encoded-params> --ua <user-agent>');
        process.exit(2);
    }
    try {
        process.stdout.write(get_ab(query, ua));
    } catch (error) {
